...
impl Meepo {
    ...
    pub fn execute_cross(&self, receipts: &Vec<TypedReceipt> , state_db: &mut StateDB, header: &Header, transactions: &Vec<SignedTransaction>,  env_info: &EnvInfo, batch: &mut DBTransaction) -> Result<(), ::error::Error> {
        ...
        //let mut total_error_txs = Vec::<H256>::new();

        self.translate_and_send(receipts, &mut state, header, transactions, env_info, batch, 0)?;
        self.withdraw_and_execute(receipts, &mut state, header, transactions, env_info, batch, 0)?;
    ...
    pub fn withdraw_and_execute(&self, receipts: &Vec<TypedReceipt> , state: &mut State<StateDB>, header: &Header, transactions: &Vec<SignedTransaction>,  env_info: &EnvInfo, batch: &mut DBTransaction, epoch_number: u64) -> Result<(), ::error::Error> {
        ...
        if !all_finished {
            ...
            self.send(&mut send_epoch_packet_vec, local_finished);
            self.withdraw_and_execute(receipts, state, header, transactions, env_info, batch, epoch_number+1);
        }
    ...
    pub fn translate_and_send(&self, receipts: &Vec<TypedReceipt> , state: &mut State<StateDB>, header: &Header, transactions: &Vec<SignedTransaction>,  env_info: &EnvInfo, batch: &mut DBTransaction, epoch_number: u64) -> Result<(), ::error::Error> {
        let start_time = Instant::now();
        ...
        for tx_index in 0..tx_len {
            ...
            self.parse_logs(&receipt.logs, &mut send_epoch_packet_vec, &mut local_finished, tx_hash);
        }
    ...
    pub fn send(&self, send_epoch_packet_vec: &mut Vec<EpochPacket>, local_finished: bool) {
